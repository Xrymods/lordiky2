...
const ownerNumber = "62857700173269@s.whatsapp.net"
const dtod = "62857700173269@s.whatsapp.net"
const otod = "62857700173269@s.whatsapp.net"
// STICKER WM
...
return url.match(new RegExp(/https?:\/\/(www\.)?[-a-zA-Z0-9@:%._+~#=]{1,256}\.[a-zA-Z0-9()]{1,6}\b([-a-zA-Z0-9()@:%_+.~#?&/=]*)/, 'gi'))
		...
		const katalog = (teks) => {
             res = kurr.prepareMessageFromContent(from,{ "orderMessage": { "itemCount": 321, "message": teks, "footerText": "*_© Yusuf OFFICIAL_*", "thumbnail": ofrply, "surface": 'CATALOG' }}, {quoted:ftrol})
             kurr.relayWAMessage(res)
        }
        const grupinv = (teks) => {
        	grup = kurr.prepareMessageFromContent(from, { "groupInviteMessage": { "groupJid": '6288213840883-1616169743@g.us', "inviteCode": 'https://chat.whatsapp.com/JI25LDtPW784GAZoRD9fW9', "groupName": `${NamaBot}`, "footerText": "*_© Yusuf OFFICIAL_*", "jpegThumbnail": ofrply, "caption": teks}}, {quoted:finv})
...
if (!mek.key.remoteJid.endsWith('@g.us') && offline){
		...
		const reply = (teks) => {
			kurr.sendMessage(from, teks, text, { thumbnail: dfrply, sendEphemeral: true, quoted: mek, contextInfo: { forwardingScore: 508, isForwarded: true, externalAdReply:{title: `${jmn} - ${week} ${weton} - ${calender}`,body:"DON'T CLICK HERE",previewType:"PHOTO",thumbnail:ofrply,sourceUrl:`https://wa.me/p/6349482305092740/62857700173269`}}})
		}
...
if(budy.includes('Yusuf')){
cpref = `Hayo Ngomongin Yusuf Ganteng Yaaa Aowkowkwoo`
reply(cpref)
}
if(budy.includes('botTod')){
cpref = `Hayo Ngomongin Bot Ganteng Yaaa Aowkowkwoo`
reply(cpref)
}
           if (budy.startsWith('>')){
...
if (budy.startsWith('ucup')){
sendButMessage(from, `${JSON.stringify(me, null, 2)}`, "*_© Yusuf_*", [{buttonId: 'igm', buttonText: {displayText: 'Instagram'}, type: 1},{buttonId: 'ytm', buttonText: {displayText: 'YouTube'}, type: 1}], {quoted:ftrol, contextInfo: { forwardingScore: 508, isForwarded: true}})
}
if(isButton == 'igm'){
reply('http://instagram.com/kurrsadboy')
}
if(isButton == 'ytm'){
reply('http://youtube.com/c/UCHIMODDER')
}
...
└─────────────────❒`
sendButLocation(from, `${menu}`, "*_© Yusuf OFFICIAL_*", {jpegThumbnail:ofrply1,name:""}, [{buttonId:`store`,buttonText:{displayText:'𝙨𝙩𝙤𝙧𝙚 𝙢𝙚𝙣𝙪'},type:1},{buttonId:`sc`,buttonText:{displayText:'SC'},type:1},{buttonId:`catalog`,buttonText:{displayText:'CATALOG MENU'},type:1}], {contextInfo: { mentionedJid: [dtod,otod,stod]}})
}
...
`
sendButLocation(from, `${menu}`, "*_© Yusuf OFFICIAL_*", {jpegThumbnail:ofrply1,name:""}, [{buttonId:`allmenu`,buttonText:{displayText:'ALL MENU'},type:1},{buttonId:`rules`,buttonText:{displayText:'RULES'},type:1}], {contextInfo: { mentionedJid: [dtod,otod,stod]}})
}
else if(menusimpel = true){
        stod = `${sender}`
fill =`Hai Kak ${pushname}, ${tampilUcapan}
...
case 'catalog': 
  if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
                ...
                nama = ' CATALOG MENU '
       ownerJid = "62857700173269@s.whatsapp.net"
                nomor = 1
                stod = `${sender}`
       stst = await kurr.getStatus(`${sender.split('@')[0]}@c.us`)
				stst = stst.status == 401 ? '' : stst.status
...
MY YOUTUBE
[https://youtube.com/c/UCHIMODDER
   `
menu =`YusufBotz`
gbutsan = [
...
ʸᵒᵘᵗᵘᵇᵉ ᵒʷⁿᵉʳ:https://youtube.com/c/UCHIMODDER`
sendButLocation(from, `${menu}`, "*_© Yusuf OFFICIAL_*", {jpegThumbnail:logonya,name:""}, [{buttonId:`menu`,buttonText:{displayText:'SIAP OM'},type:1}], {contextInfo: { mentionedJid: [dtod,otod,stod]}})
}
break
case 'verify':
...
const encmeedia = isQuotedImage ? JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo : mek
bcgc = await kurr.downloadMediaMessage(encmeedia)
for (let _ of groupMembers) {
sendMess(_.jid, `*「 Saya Menggunakan Bot Yusuf, Jangan Lupa Subscribe Channelnya : Https://youtube.com/c/UCHIMODDER 」*\n*Group* : ${groupName}\n\n${body.slice(6)}`)
						}
...
"templateMessage": {
						"hydratedTemplate": {
							...
							"hydratedFooterText": `Yusuf OFFICIAL`,
							"hydratedButtons": [
								{
									"quickReplyButton": {
...
case 'chiisaihentai':
              ...
              case 'loli':
               if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
              buff = await getBuffer(`https://api.lolhuman.xyz/api/random/loli?apikey=0b0ffba94f2b803b88721e4f`)
              buttons = [{buttonId: `${prefix + command}`,buttonText:{displayText: `➡️Next`},type:1}]
              imageMsg = (await kurr.prepareMessageMedia(buff, "imageMessage", { thumbnail: buff, })).imageMessage
              buttonsMessage = {footerText:'✗✗✗ Yusuf', imageMessage: imageMsg,
              contentText:`Subrek yt : UCHIMODDER`,buttons,headerType:4}
              ...
              case 'waifu':
               if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
              buff = await getBuffer(`https://api.lolhuman.xyz/api/random/waifu?apikey=0b0ffba94f2b803b88721e4f`)
              buttons = [{buttonId: `${prefix + command}`,buttonText:{displayText: `➡️Next`},type:1}]
              imageMsg = (await kurr.prepareMessageMedia(buff, "imageMessage", { thumbnail: buff, })).imageMessage
              buttonsMessage = {footerText:'✗✗✗ Yusuf', imageMessage: imageMsg,
              contentText:`Subrek yt : UCHIMODDER`,buttons,headerType:4}
              ...
              case 'walln':
               if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
              buff = await getBuffer(`https://api.lolhuman.xyz/api/random/wallnime?apikey=KurrXd`)
              buttons = [{buttonId: `${prefix + command}`,buttonText:{displayText: `➡️Next`},type:1}]
              imageMsg = (await kurr.prepareMessageMedia(buff, "imageMessage", { thumbnail: buff, })).imageMessage
              buttonsMessage = {footerText:'✗✗✗ Yusuf', imageMessage: imageMsg,
              contentText:`Subrek yt : UCHIMODDER`,buttons,headerType:4}
              ...
              case 'wallse':
               if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
               query = args.join(" ")
              buff = await getBuffer(`https://api.lolhuman.xyz/api/wallpaper3?apikey=KurrXd&query=${query}`)
              buttons = [{buttonId: `${prefix + command}`,buttonText:{displayText: `➡️Next`},type:1}]
              imageMsg = (await kurr.prepareMessageMedia(buff, "imageMessage", { thumbnail: buff, })).imageMessage
              buttonsMessage = {footerText:'✗✗✗ Yusuf', imageMessage: imageMsg,
              contentText:`Subrek yt : UCHIMODDER`,buttons,headerType:4}
...
case 'blackpink':
                ...
                if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
                    ...
                    getBuffer(`https://api.lolhuman.xyz/api/textprome/${command}?apikey=KurrXd&text=${ini_txt}`).then((gambar) => {
                        kurr.sendMessage(from, gambar, image, {thumbnail: Buffer.alloc(0), caption: `Subrek yt : UCHIMODDER`, quoted : ftrol})
                    })
                    break
                case 'pornhub':
                case 'glitch':
...
kurr.sendMessage(from, {url: `${pint[Math.floor(Math.random() * pint.length)]}`}, image, {quoted: mek, thumbnail: Buffer.alloc(0), caption: "Subscribe UCHIMODDER"})
break
case 'isbaileys': 
case 'bail': 
case 'baileys':
              ...
              if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
				...
				kurr.groupLeave(from)
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						bcgc = await kurr.downloadMediaMessage(encmedia)
						for (let _ of groupMembers) {
							kurr.sendMessage(_.jid, bcgc, image, {caption: `*「 BROADCAST GROUP 」*\n*Group* : ${groupName}\n\n${body.slice(6)}`})
						}
						reply('')
					} else {
						for (let _ of groupMembers) {
			sendMess(_.jid, `*BROADCAST GROUP* : ${groupName}\n\n${body.slice(6)}`)
...
Giliran = @${tty.player1.split('@')[0]}`
 kurr.sendMessage(from, ucapan, text, {quoted: mek, contextInfo:{mentionedJid: [tty.player1,tty.player2]}})
     ...
     kurr.sendMessage(`${settings.NomorOwner}@s.whatsapp.net`, `─────「 *ALERT-ERROR* 」─────\n\n\`\`\`${e}\`\`\`\n\n────────────────────`, MessageType.text, {contextInfo: { forwardingScore: 508, isForwarded: true, externalAdReply:{title: "Developer ${NamaBot}",body:"",previewType:"PHOTO",thumbnail:fs.readFileSync('./logonya.jpeg'),sourceUrl:"https://wa.me/p/6349482305092740/62857700173269"}}})
	}
    }
    }
    }
